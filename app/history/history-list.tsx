...
import { 
  Search, Calendar, MapPin, ArrowUpDown, Bike, ChevronRight, Clock, 
  ArrowLeftRight, CheckCircle2, Circle, X, Crown, Star, Trophy
} from "lucide-react"
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
            ...
            {hasActiveFilters && (
              <Button 
                variant="destructive" 
                size="icon" 
                className="h-9 w-9 shrink-0 rounded-lg opacity-90 hover:opacity-100 transition-opacity"
            ...
            <Select value={filterCircuit} onValueChange={setFilterCircuit}>
              <SelectTrigger className="h-9 text-xs bg-white dark:bg-slate-950 min-w-[110px] max-w-[140px]">
                <div className="flex items-center gap-2 text-slate-500 truncate w-full">
                   <MapPin size={14} className="shrink-0" /> 
                   <span className="truncate block">{filterCircuit === 'all' ? 'Tutti' : filterCircuit}</span>
                </div>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Tutti i Circuiti</SelectItem>
                {uniqueCircuits.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <Select value={filterBike} onValueChange={setFilterBike}>
              <SelectTrigger className="h-9 text-xs bg-white dark:bg-slate-950 min-w-[110px] max-w-[140px]">
                <div className="flex items-center gap-2 text-slate-500 truncate w-full">
                   ...
                   <span className="truncate block">{filterBike === 'all' ? 'Tutte' : filterBike}</span>
                </div>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Tutte le Moto</SelectItem>
                {uniqueBikes.map(b => <SelectItem key={b} value={b}>{b}</SelectItem>)}
              </SelectContent>
            </Select>
            
            <Button 
               variant="ghost" size="icon" className="h-9 w-9 shrink-0 ml-auto border border-slate-200 dark:border-slate-800"
                ...
                <div key={groupKey} className="relative">
                  ...
                  <div className="sticky top-[148px] z-20 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-sm py-2 px-1 mb-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-800 dark:text-white capitalize text-sm sm:text-base">
                          {formatDate(dateStr)}
                        </h3>
                        <div className="flex items-center gap-1 text-xs text-slate-500">
                          <MapPin size={12} /> {circuitName}
                        </div>
                      </div>
                    </div>
                    
                    {sessions.some(s => s.isBestDay) && (
                       <Badge variant="secondary" className="hidden xs:flex bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-[10px] gap-1 border-none">
                          <Star size={10} fill="currentColor" /> Best Day
                       </Badge>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 pl-2 sm:pl-4 border-l-2 border-slate-200 dark:border-slate-800 ml-2 sm:ml-4">
                    {sessions.map((session) => (
                      <div key={session.id} className="relative flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        {isSelectionMode && (
                          <button onClick={() => toggleSelection(session.id)} className="shrink-0 transition-transform active:scale-90">
                             {selectedIds.includes(session.id) 
                               ...
                               : <Circle className="text-slate-300 h-6 w-6" />
                             }
                          </button>
                        )}
                        ...
                        >
                          <Card className={cn(
                            "transition-all dark:bg-slate-900 dark:border-slate-800 relative overflow-hidden",
                            isSelectionMode && selectedIds.includes(session.id) 
                                ? "ring-2 ring-green-500 border-transparent bg-green-50 dark:bg-green-900/10" 
                                : "hover:border-green-500 hover:shadow-md"
                          )}>
                            
                            {session.isBestCircuit && (
                               <div className="absolute right-0 top-0 p-1.5 bg-yellow-400 text-yellow-900 rounded-bl-xl z-10 shadow-sm" title="Best Lap del Circuito">
                            ...
                            <CardContent className="p-3 sm:p-4 flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                                 <div className="flex flex-col items-center justify-center w-12 h-12 bg-slate-100 dark:bg-slate-950 rounded-lg text-slate-500 shrink-0">
                                 ...
                                 <div className="min-w-0 flex-1">
                                   <h4 className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-green-600 transition-colors truncate text-sm sm:text-base">
                                     {session.name}
                                   </h4>
                                   <div className="flex flex-wrap items-center gap-2 mt-1">
                                     {session.track_days?.bike && (
                                       <span className="text-[10px] text-slate-500 flex items-center gap-1 bg-slate-50 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-100 dark:border-slate-700 max-w-full">
                                          ...
                                          <span className="truncate">{session.track_days.bike.model}</span>
                                       </span>
                                     )}
                                     
                                     {session.isBestBike && (
                                        <span className="text-[9px] font-bold text-blue-500 flex items-center gap-0.5 bg-blue-50 dark:bg-blue-900/20 px-1 rounded">
                                            <Trophy size={10}/> Moto
                                        </span>
                                     )}
                                     {session.isBestDay && (
                                        <span className="text-[9px] font-bold text-amber-500 flex items-center gap-0.5 bg-amber-50 dark:bg-amber-900/20 px-1 rounded">
                                            <Star size={10}/> Day
                                        </span>
                                     )}
                                   </div>
                              ...
                              <div className="text-right shrink-0 pl-2">
                                 {session.best_lap ? (
                                   <div className={cn(
                                     ...
                                     session.isBestCircuit ? "text-yellow-600 dark:text-yellow-400" : "text-slate-900 dark:text-white"
                                   )}>
                                     {session.best_lap}
                                   </div>
                                 ) : (
                                   <span className="text-xs text-slate-400 italic">No time</span>
                                 )}
                                 {!isSelectionMode && (
                                    <ChevronRight className="ml-auto mt-1 text-slate-300 group-hover:text-green-500 transition-colors" size={16} />
                                 )}
                              </div>
